...
void DotSceneLoader::processParticleSystem(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    try
    {
        ...
        particleSystemObjects[name] = pParticles;
...
void DotSceneLoader::processBillboardSet(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    Ogre::String name = getAttrib(XMLNode, "name");
    Ogre::String id = getAttrib(XMLNode, "id");
    Ogre::String type = getAttrib(XMLNode, "type");
    Ogre::String origin = getAttrib(XMLNode, "origin");
    ...
    try
    {
        Ogre::BillboardSet* bSet = mSceneMgr->createBillboardSet(name);
        billboardSetObjects[name] = bSet;

        if (type == "orientedCommon")
           bSet->setBillboardType(Ogre::BBT_ORIENTED_COMMON);
        else if (type == "orientedSelf")
           bSet->setBillboardType(Ogre::BBT_ORIENTED_SELF);
        else
           bSet->setBillboardType(Ogre::BBT_POINT);
        
        if (origin == "bottom_left")
           bSet->setBillboardOrigin(Ogre::BBO_BOTTOM_LEFT);
        else if (origin == "bottom_center")
           bSet->setBillboardOrigin(Ogre::BBO_BOTTOM_CENTER);
        else if (origin == "bottomRight")
           bSet->setBillboardOrigin(Ogre::BBO_BOTTOM_RIGHT);
        else if (origin == "left")
           bSet->setBillboardOrigin(Ogre::BBO_CENTER_LEFT);
        else if (origin == "right")
           bSet->setBillboardOrigin(Ogre::BBO_CENTER_RIGHT);
        else if (origin == "topLeft")
           bSet->setBillboardOrigin(Ogre::BBO_TOP_LEFT);
        else if (origin == "topCenter")
           bSet->setBillboardOrigin(Ogre::BBO_TOP_CENTER);
        else if (origin == "topRight")
           bSet->setBillboardOrigin(Ogre::BBO_TOP_RIGHT);
        else
           bSet->setBillboardOrigin(Ogre::BBO_CENTER);

        bSet->setMaterialName(name);
        bSet->setVisible(true);

        pParent->attachObject(bSet);

        rapidxml::xml_node<>* XMLChildNode = XMLNode->first_node("billboard");
        ...
        while (XMLChildNode)
        {
           ...
           rapidxml::xml_node<>* XMLPosNode = XMLChildNode->first_node("position");
           if (XMLPosNode)
              vec = parseVector3(XMLPosNode);
            
           rapidxml::xml_node<>* XMLColorNode = XMLChildNode->first_node("colourDiffuse");
           if (XMLColorNode)
              color = parseColour(XMLColorNode);

           b = bSet->createBillboard(vec, color);

           XMLChildNode = XMLChildNode->next_sibling("billboard");
        }
    }
    catch(Ogre::Exception &)
    {
        Ogre::LogManager::getSingleton().logMessage("[DotSceneLoader] Error creating a billboard set!");
    }
}
...
void DotSceneLoader::processPlane(rapidxml::xml_node<>* XMLNode, Ogre::SceneNode *pParent)
{
    ...
    Ogre::MeshPtr res = Ogre::MeshManager::getSingletonPtr()->createPlane(name + "mesh", m_sGroupName, plane, width, height, xSegments, ySegments, hasNormals,
    numTexCoordSets, uTile, vTile, up);
    Ogre::Entity* ent = mSceneMgr->createEntity(name, name + "mesh");  
    entityObjects[name] = ent;
